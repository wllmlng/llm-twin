...
def main(
    ...
    if run_end_to_end_data:
        ...
        pipeline_args["run_name"] = (
            f"end_to_end_data_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
        )
    ...
    if run_generate_preference_datasets:
        ...
        pipeline_args["run_name"] = (
            f"generate_preference_datasets_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
        )
        generate_datasets.with_options(**pipeline_args)(**run_args_cd)
    ...
    if run_evaluation:
        ...
        pipeline_args["run_name"] = (
            f"evaluation_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
        )
